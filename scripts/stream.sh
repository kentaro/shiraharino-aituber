...
if [[ "$CHROME_USE_SWIFTSHADER" == "1" ]]; then
  ...
  CHROME_RENDER_FLAGS=( --use-gl=swiftshader --enable-unsafe-swiftshader --disable-vulkan )
else
  CHROME_PROCESS_FLAGS=( --single-process --renderer-process-limit=1 --disable-site-isolation-trials )
  CHROME_RENDER_FLAGS=( --disable-gpu --disable-gpu-compositing --disable-software-rasterizer --disable-vulkan )
fi
...
if [[ "$VIDEO_CLOCK_BASE" == "1" && ! -f "$BGM_FILE" ]]; then
  ...
  AV_FILTER_MAP=( -filter_complex "[0:v]scale=${WIDTH}:${HEIGHT}:flags=neighbor[grab];[2:v][grab]overlay=shortest=0:repeatlast=1,${VIDEO_FILTER}[vout]" -map "[vout]" -map 1:a:0 )
fi
