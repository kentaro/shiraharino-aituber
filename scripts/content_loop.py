...
TARGET = int(os.environ.get("TARGET", "6"))           # 先読みバッファ本数
...
def main():
    ...
    while True:
        ...
        if not text:
            fail_streak += 1
            if fail_streak >= int(os.environ.get("FALLBACK_AFTER_EMPTY", "1")):
                text = fallback_line(recent)
                if text:
                    sys.stderr.write(f"[content] gen fallback after empty={fail_streak}: {text}\n")
                    fail_streak = 0
                else:
                    back = min(int(os.environ.get("GEN_EMPTY_MAX_WAIT", "2")), 2 * fail_streak)
